  ...
  return (
    <PageWrapper>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {[
          { title: "Student Nodes", href: "/admin/cohorts", desc: "Batch import, audit, and manipulate student profile data and competencies.", icon: "🧑‍🎓", color: "text-indigo-400", border: "border-indigo-500" },
          { title: "Company Taxonomy", href: "/admin/analytics", desc: "Define hiring partners, job roles, and map required skills to companies.", icon: "🏢", color: "text-cyan-400", border: "border-cyan-500" },
          { title: "Skill Ontology", href: "/admin/analytics", desc: "Curate the global graph of skills, relationships, and mastery levels.", icon: "⚡", color: "text-amber-400", border: "border-amber-500" },
          { title: "Placement Ledger", href: "/admin/analytics", desc: "Immutable records of alumni transitions and current active placements.", icon: "📋", color: "text-emerald-400", border: "border-emerald-500" },
          { title: "Neo4j Config", href: "/admin/analytics", desc: "Monitor Cypher queries, graph visualization settings, and database limits.", icon: "🔗", color: "text-rose-400", border: "border-rose-500" },
          { title: "System Logs", href: "/admin/analytics", desc: "Check server health, API rate limits, and access administrative audit logs.", icon: "⚙️", color: "text-slate-400", border: "border-slate-500" },
        ].map((item) => (
          <Card key={item.title} hover className={`relative overflow-hidden group border-t-2 ${item.border}`}>
            ...
            <div className="mt-6">
              <Link href={item.href} className="text-[10px] uppercase font-bold tracking-widest text-indigo-400 group-hover:text-indigo-300 transition-colors">
                Access Module →
              </Link>
            </div>
          </Card>
