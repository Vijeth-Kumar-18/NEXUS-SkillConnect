...
function loadDataset(fileName: string): string {
  const candidates = [
    ...
    path.join(process.cwd(), "datasets", fileName),
...
function loadFilteredJsonDataset(): { companies: NormalizedCompany[]; students: NormalizedStudent[]; alumni: NormalizedAlumni[] } | null {
  const candidates = [
    path.join(process.cwd(), "filtered-data", "filtered-dataset.json"),
    path.join(process.cwd(), "filtered-data", "filtered-neo4j-dataset.json"),
  ];
  for (const filePath of candidates) {
    if (fs.existsSync(filePath)) {
      try {
        ...
        if (parsed && Array.isArray(parsed.companies) && Array.isArray(parsed.students) && Array.isArray(parsed.alumni)) {
          return {
...
export async function seedFromLocalDatasets(): Promise<{ companies: number; students: number; alumni: number }> {
  ...
  const filteredJson = loadFilteredJsonDataset();
  let companyRows: NormalizedCompany[] = [];
  let studentRows: NormalizedStudent[] = [];
  let alumniRows: NormalizedAlumni[] = [];

  if (filteredJson) {
    companyRows = filteredJson.companies;
    studentRows = filteredJson.students;
    alumniRows = filteredJson.alumni;
  } else {
    companyRows = normalizeCompanyRows(loadDataset("Data1.txt"));
    studentRows = normalizeStudentRows(loadDataset("Data2.txt"));
    alumniRows = normalizeAlumniRows(loadDataset("Data3.txt"));
  }
